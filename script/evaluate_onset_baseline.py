...
def eval_osnets(onset1, onset2, wav2, delta=0.1, conf_interval=int(0.05*22050), keys=None):
    ...
    if keys != None and args.plt:
        plt.plot(rc, pr)
        plt.ylim((0, 1))
        plt.savefig(f'/homes/emilpost/riccardo/diffusion-sfx/temp_transformer/pc_rc_curve_tar_{keys[0]}_cond_{keys[1]}.jpg')
        plt.close()
...
if __name__ == '__main__':
    ...
    if args.plt:
        plot_dir = os.path.join('/homes/emilpost/riccardo/diffusion-sfx/onset_detection_vis_transformer/', args.gen_dir.split('/')[-1])
    ...
    for target_video_name in os.listdir(args.tar_dir):
        if target_video_name.endswith('.mp4'):
            target_video_name_without_extension, ext = os.path.splitext(target_video_name)
            target_video_names.add(target_video_name_without_extension)

    for generated_video_name in tqdm(onset_list2.keys()):
        generated_video_name_without_extension, ext = os.path.splitext(generated_video_name)
        
        generated_video_parts = generated_video_name_without_extension.split("_to_")
        
        if len(generated_video_parts) == 2 and generated_video_parts[0] in target_video_names:
            target_video_name = generated_video_parts[0] + '.mp4'
            
            tar_key = target_video_name.split('.')[0]
            
            onset1 = onset_list1[tar_key]
            onset2 = onset_list2[generated_video_name]
            ...
            if args.multi_delta:
                ...
                for delta in delta_list:
                    _acc, _ap, onset2_res = eval_osnets(onset1, onset2, wav_list2[generated_video_name], delta=delta, keys=(tar_key, generated_video_name))
                    acc += _acc
                    ap += _ap
                acc /= len(delta_list)
                ap /= len(delta_list)
            else:
                acc, ap, onset2_res = eval_osnets(onset1, onset2, wav_list2[generated_video_name], delta=args.delta, keys=(tar_key, generated_video_name))
            if args.plt:
                plot_onset(os.path.join(plot_dir, f'onset_wav_tar_{tar_key}_cond_{generated_video_name}.jpg'), wav_list1[tar_key], wav_list2[generated_video_name], onset1, onset2, pred=onset2_res, ap=ap)
            onset_ap.append(ap)
