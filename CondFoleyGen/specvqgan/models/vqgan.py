...
from CondFoleyGen.specvqgan.data.transforms import Wave2Spectrogram, NormalizeAudio
from CondFoleyGen.specvqgan.modules.diffusionmodules.model import Encoder, Decoder, Encoder1d, Decoder1d
from CondFoleyGen.specvqgan.modules.vqvae.quantize import VectorQuantizer, VectorQuantizer1d
...
class VQModel(pl.LightningModule):
    ...
    #     print("VQGAN - TRAINING STEP")
    #     print(f"OPTIMIZER IDX: {optimizer_idx}")
    #     x = self.get_input(batch, self.image_key)
    #     xrec, qloss = self(x)
    ...
    def validation_step(self, batch, batch_idx):
        if batch_idx == 0 and self.global_step != 0 and sum(self.counts) > 0:
            ...
            self.logger.experiment.log(
                {"val/code_hits": wandb.Histogram(torch.tensor(used_codes))},
            ...
            # )
            self.logger.experiment.log(
                {"val/zero_hit_codes": zero_hit_codes},
                step=self.global_step
            )
