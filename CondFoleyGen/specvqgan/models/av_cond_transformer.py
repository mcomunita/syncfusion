...
from CondFoleyGen.specvqgan.modules.transformer.mingpt import GPTClass, GPTFeats, GPTFeatsClass
from CondFoleyGen.specvqgan.data.transforms import Wave2Spectrogram, PitchShift, NormalizeAudio
...
class Net2NetTransformerAVCond(pl.LightningModule):
    def __init__(
        ...
        drop_video=False,
        drop_cond_video=False,
        first_stage_permuter_config=None,
        cond_stage_permuter_config=None,
        ckpt_path=None,
        ignore_keys=[],
        ...
        print("\ninstantiating first stage model")
        self.init_first_stage_from_ckpt(first_stage_config)

        print("\ninstantiating conditioning stage model")
        self.init_cond_stage_from_ckpt(cond_stage_config)

        print(f"\nfirst stage permuter config: {first_stage_permuter_config}")
        if first_stage_permuter_config is None:
            first_stage_permuter_config = {"target": "CondFoleyGen.specvqgan.modules.transformer.permuter.Identity"}
        
        print(f"\ncond stage permuter config: {cond_stage_permuter_config}")
        if cond_stage_permuter_config is None:
            cond_stage_permuter_config = {"target": "CondFoleyGen.specvqgan.modules.transformer.permuter.Identity"}
        
        print("\ninstantiating first stage permuter")
        self.first_stage_permuter = instantiate_from_config(config=first_stage_permuter_config)

        print("\ninstantiating conditioning stage permuter")
        self.cond_stage_permuter = instantiate_from_config(config=cond_stage_permuter_config)
        # self.first_stage_permuter = first_stage_permuter_config
        # self.cond_stage_permuter = cond_stage_permuter_config
        ...
        print(f'\n>>> Feature setting: all cond: {self.drop_condition}, video: {self.drop_video}, cond video: {self.drop_cond_video}')
    ...
    def forward(self, x, c, xp):
        ...
        _, z_indices = self.encode_to_z(x)  # encode ref audio tokens w/ VQ-GAN
        _, zp_indices = self.encode_to_z(xp) # encode cond audio tokens w/ VQ-GAN
        _, c_indices = self.encode_to_c(c)  # encode ref + cond frames w/ GPT (Conv1-1 down dim + col-major permuter)
    ...
    def spec_transform(self, batch):
        wav = batch[self.first_stage_key] # get ref audio
        wav_cond = batch[self.cond_first_stage_key] # get conditioning audio
        N = wav.shape[0]
    ...
    def get_input(self, key, batch):
        if isinstance(key, str):
            if key in ['feature', 'target']: # batch[key] is 1D
    ...
    def get_xcxp(self, batch, N=None):
        ...
        # if audio input convert to spectrogram
