...
from CondFoleyGen.feature_extraction.extract_mel_spectrogram import inv_transforms
from CondFoleyGen.specvqgan.models.vocoder_melgan import Generator
from CondFoleyGen.specvqgan.utils import get_ckpt_path, instantiate_from_config
...
def load_model_from_config(config, state_dict, gpu=True, eval_mode=True, load_new_first_stage=False):
    print("\nLOAD_MODEL_FROM_CONFIG")
    pprint(config)

    if "ckpt_path" in config["init_args"]:
        print("Deleting the restore-ckpt path from the config...")
        config["init_args"]["ckpt_path"] = None
    try:
        if "ckpt_path" in config["init_args"]["first_stage_config"]["params"] and not load_new_first_stage:
            config["params"]["first_stage_config"]["params.ckpt_path"] = None
            print("Deleting the first-stage restore-ckpt path from the config...")
        if "ckpt_path" in config["params"]["cond_stage_config"]["params"]:
            config["params"]["cond_stage_config"]["params"]["ckpt_path"] = None
    ...
    config["target"] = config["class_path"]
    del config["class_path"]
    config["params"] = config["init_args"]
    del config["init_args"]
...
def spec_to_audio_to_streamlit(
    x, 
    spec_dir_path, 
    sample_rate, 
    show_griffin_lim, 
