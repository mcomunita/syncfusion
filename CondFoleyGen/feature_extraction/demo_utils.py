...
from CondFoleyGen.specvqgan.utils import download, md5_hash
from CondFoleyGen.audio_generation_utils import get_melspectrogram_torch
from CondFoleyGen.feature_extraction.extract_mel_spectrogram_greatesthit import get_spectrogram_infer
from CondFoleyGen.feature_extraction.extract_mel_spectrogram import get_spectrogram as get_spectrogram_non_normalize
...
def load_config(config_path):
    return yaml.load(open(config_path, 'r'), Loader=yaml.UnsafeLoader)
...
def load_model(
    ...
    device,
    load_feat_extractor,
    load_new_first_stage=False,
):
    print('\nLOAD_MODEL')
    print('model_path:', model_path)
    print('config_path:', config_path)
    print('device:', device)
    print('load_new_first_stage:', load_new_first_stage)
    ...
    sampler = load_model_from_config(
        ...
        to_use_gpu, 
        eval_mode=True,
        load_new_first_stage=load_new_first_stage
    )['model']
    sampler.to(device)
    ...
    for cb in config["trainer"]["callbacks"]:
        if cb["class_path"] == "CondFoleyGen.specvqgan.modules.callbacks.image_logger.ImageLogger":
            ckpt_melgan = cb["init_args"]["vocoder_cfg"]["params"]["ckpt_vocoder"]
    melgan = load_vocoder(ckpt_melgan, eval_mode=True)['model'].to(device)

    if load_feat_extractor:
        print("\nLoading Feature Extractor")
        melception = load_feature_extractor(to_use_gpu, eval_mode=True)
        return config, sampler, melgan, melception
    
    return config, sampler, melgan
...
def extract_melspectrogram(
    ...
    if using_torch:
        spec = get_melspectrogram_torch(
            audio, 
            save_dir=None, 
            length=length
        )
    elif normalize:
        audio_zero_pad, spec = get_spectrogram_infer(
            audio, 
            save_dir=None, 
            length=length, 
            save_results=False
        ...
        audio_zero_pad, spec = get_spectrogram_non_normalize(
            audio, 
            save_dir=None, 
            length=length, 
            save_results=False
